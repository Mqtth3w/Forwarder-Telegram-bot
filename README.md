...
- To delete a message do reply at the message with the ID of the sender with `/del <ID of message to be deleted>`. Understand the ID may be difficult. Once a user type you, you get a subsequent message with their info and the ID of the message that the user sent, if you reply immeadiately the user and you want to delete the reply then you need to reply to the message with the ID of sender with `/del <prev_msg_id + 5>` this is because the IDs are incremental: +1 because the bot send to user "message sent", +2 becusse the message is forwared to you, +3 because the subsequent info message, +4 for your reply in the bot chat, then +5 is your reply sent to that user. If you wait and do not delete the message immediately then you need to count all messages into the middle (from the "message sent" of the bot to your reply +1). There are no other ways. (You can also delete the messages sent by the bot, by the user, etc, the whole chat one by one).
